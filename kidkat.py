...
####### Load Datasets #####################
DF_RES = pd.read_json("kitkat_dataset/kitkat_replies.json")
DF_RES["timestamp"] = pd.to_datetime(DF_RES["timestamp"])

col = ['id', 'text', 'created_at']
DF_SENT = pd.read_csv(
    "kitkat_dataset/kitkat_tweets.csv", header=None, names=col)
...
DF_HASH = pd.read_json("kitkat_dataset/kitkat_hashtag.json")
DF_HASH["timestamp"] = pd.to_datetime(DF_HASH["timestamp"])

DF_ENG = pd.read_json("kitkat_dataset/kitkat_engagement.json")
...
with col1_2:  
        ...
        ax.set_title('Kitkat Response Time Distribution')
...
st.markdown('<h5 style="text-align:center; color:red">Hashtag: <b>#HaveABreak</b></h5>', unsafe_allow_html=True)
