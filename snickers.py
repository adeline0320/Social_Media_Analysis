...
######################## Load Datasets ##########################
DF_RES = pd.read_json("snickers_dataset/snickers_replies_3.json")
DF_RES["timestamp"] = pd.to_datetime(DF_RES["timestamp"])

col = ['id', 'text', 'created_at']
DF_SENT = pd.read_csv(
    "snickers_dataset/snickers_tweets.csv", header=None, names=col)
...
DF_HASH = pd.read_json("snickers_dataset/snickers_hashtag.json")
DF_HASH["timestamp"] = pd.to_datetime(DF_HASH["timestamp"])

DF_ENG = pd.read_json("snickers_dataset/snickers_engagement.json")
...
reply = DF_RES[["text","username","timestamp"]]
...
for index, row in reply.iterrows():
  if row['username'] == "@SNICKERS":
    snickers_reply.append(DF_RES.loc[index])

# print(len(reply["in_reply_to"])!=0)

time_list = [0,0,0,0,0,0]
time_range = ["0-60 minutes","1-2 hours","2-6 hours","6-12 hours","12-24 hours","24 hours+"]

for index, name in reply.iterrows():
  for index2, reply in enumerate(snickers_reply):
...
with col1_1:
    st.markdown("**Average response time in:**")
    # st.write("Average reply per day: ", round(average_seconds, 2))
    # st.write("Average reply per seconds: ", round(average_days, 2))
    ...
    st.markdown(f"<ul><li><span style='color: red;font-size: 20px'><b><i>{round(average_seconds, 2)}</i></b></span> <span style='color: blue;font-size: 20px'>seconds</span></li><li><span style='color: red;font-size: 20px'><b><i>{round(average_days, 2)}</i></b></span> <span style='color: blue;font-size: 20px'>days</span></li></ul>", unsafe_allow_html=True)
    # st.markdown(f"<li style = 'color:red;'>{round(average_days, 2)}</li></ul>", unsafe_allow_html=True)
    # st.markdown("""
    #         * {} seconds
    #         * {} days
    # """.format(round(average_seconds, 2), round(average_days, 2)))
...
with col1_2:  
        ...
        ax.set_title('Snickers Response Time Distribution')
...
with container1:
    with col1:
        ...
        # ax.set_title('Total Snickers Engagement over Time')
        ax.tick_params(axis='x', rotation=45)
...
st.markdown('<h5 style="text-align:center; background-color:#1338BE; color:white;">Hashtag: <b>#SnickersIceCream</b></h5>', unsafe_allow_html=True)
